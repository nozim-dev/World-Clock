...
function updateTime1(dataCountry) {
    dataCountry.map((country) => {
        fetch(`https://timeapi.io/api/time/current/zone?timeZone=${country.qita + '%2F' + country.shahar}`).then((res) => {
            return res.json();
        }).then((data) => {
...
function searchClock() {
    ...
    fetch(`https://timeapi.io/api/time/current/zone?timeZone=${findQita}`).then((res) => {
        return res.json();
    }).then((data) => {
        if (data === "Invalid Timezone") {
            document.querySelector(".error_Message").innerHTML = "Error: The Capital of Country name is incorrect or no information was found for this city!"
        } else {
